...
public class CheckWordsterClient {
    ...
    public void startServer() throws Exception {
        if (whichServer.equals("local")) {
            String[] execStrings = null;

            if (SystemUtils.IS_OS_WINDOWS) {
                execStrings = new String[] {"java", "-jar", ".\\target\\checkwordster-spring-boot-0.0.1-SNAPSHOT-server.jar"};
            } else {
                execStrings = new String[] {"java", "-jar", "./target/checkwordster-spring-boot-0.0.1-SNAPSHOT-server.jar"};
            }

            serverRuntime = Runtime.getRuntime().exec(execStrings);
            Thread.sleep(1000);
